...
&coh∙ : ∀ {i} → {X : Type i} {a b : X} → a == b → {c : X} → b == c → a == c
&coh∙ idp idp = idp

&coh∙□ : ∀ {i} → {X : Type i} {a b : X} {p q : a == b} (α : Square p q idp idp) {r : a == b} (β : Square q r idp idp) → Square p r idp idp
&coh∙□ {p = idp} ids ids = ids

&coh!∙□ : ∀ {i} → {X : Type i} {a b : X} {q p : a == b} (α : Square q p idp idp) {r : a == b} (β : Square q r idp idp) → Square p r idp idp
...
module _ {i} {A B : Ptd i} where
  module SmashRec {P : Type i} (proj* : ∣ A ∣ → ∣ B ∣ → P)
           (basel* : P) (gluel* : (a : ∣ A ∣) → proj* a (pt B) == basel*)
           (baser* : P) (gluer* : (b : ∣ B ∣) → proj* (pt A) b == baser*)  where
...
&hids : ∀ {i} → {X : Type i} {a b : X} (p : a == b) → Square p p idp idp
&hids idp = ids

&vids : ∀ {i} → {X : Type i} {a b : X} (p : a == b) → Square idp idp p p
&vids idp = ids

&hidc : ∀ {i} → {X : Type i} {a b c d : X} {p : a == b} {q : c == d} {r : a == c} {s : b == d} {sq : Square p q r s} → Cube sq sq (&hids p) (&hids q) (&hids r) (&hids s)
&hidc {sq = ids} = idc

&vidc : ∀ {i} → {X : Type i} {a b : X} (p : a == b) → Cube ids ids (&vids p) (&vids p) (&vids p) (&vids p)
&vidc idp = idc

&hidhc : ∀ {i} → {X : Type i} {a b c d : X} {p : a == b} {q : c == d} {r : a == c} {s : b == d} {sq : Square p q r s}
                              {a' b' c' d' : X} {p' : a' == b'} {q' : c' == d'} {r' : a' == c'} {s' : b' == d'} {sq' : Square p' q' r' s'}
                              {a= : a == a'} {b= : b == b'} {c= : c == c'} {d= : d == d'}
                              {p= : Square p p' a= b=}
                              {q= : Square q q' c= d=}
                              {r= : Square r r' a= c=}
                              {s= : Square s s' b= d=}
                              {cu : Cube sq sq' p= q= r= s=}
                              → HyperCube cu cu &hidc &hidc &hidc &hidc &hidc &hidc
&hidhc {cu = idc} = idhc
...
&inv : ∀ {i} → {A : Type i} {a b : A} (p : a == b) → b == a
&inv idp = idp
...
&antidegen : ∀ {i} → {X : Type i} {a b : X} {u v : a == b} → Square u v idp idp → u == v
&antidegen {u = idp} ids = idp

&antidegen-cube : ∀ {i} → {X : Type i} {a b c d : X} {p : a == b} {q : c == d} {r : a == c} {s : b == d} {sq1 sq2 : Square p q r s} → Cube sq1 sq2 (&hids _) (&hids _) (&hids _) (&hids _) → sq1 == sq2
...
&degen : ∀ {i} → {A : Type i} {a b : A} {p q : a == b} → p == q → Square p q idp idp
&degen {p = idp} idp = ids
...
&degen! : ∀ {i} → {A : Type i} {a b : A} {p q : a == b} → p == q → Square q p idp idp
&degen! {p = idp} idp = ids
...
ap+++ : ∀ {i} {A B : Type i} {a b c d : A → B} {p : (x : A) → a x == b x} {q : (x : A) → c x == d x} {r : (x : A) → a x == c x} {s : (x : A) → b x == d x} {α : (x : A) → Square (p x) (q x) (r x) (s x)}
                             {a' b' c' d' : A → B} {p' : (x : A) → a' x == b' x} {q' : (x : A) → c' x == d' x} {r' : (x : A) → a' x == c' x} {s' : (x : A) → b' x == d' x} {α' : (x : A) → Square (p' x) (q' x) (r' x) (s' x)}
                             {a= : (x : A) → a x == a' x} {b= : (x : A) → b x == b' x} {c= : (x : A) → c x == c' x} {d= : (x : A) → d x == d' x}
                             {p= : (x : A) → Square (p x) (p' x) (a= x) (b= x)}
                             {q= : (x : A) → Square (q x) (q' x) (c= x) (d= x)}
                             {r= : (x : A) → Square (r x) (r' x) (a= x) (c= x)}
                             {s= : (x : A) → Square (s x) (s' x) (b= x) (d= x)}
                             (cu : (x : A) → Cube (α x) (α' x) (p= x) (q= x) (r= x) (s= x))
                             {x y : A} (k : x == y)
              → HyperCube (cu x) (cu y) (ap++ α k) (ap++ α' k) (ap++ p= k) (ap++ q= k) (ap++ r= k) (ap++ s= k)
ap+++ cu idp = &hidhc              
...
&aphids : ∀ {i} → {X Y : Type i} (f : X → Y) {a b : X} (p : a == b) → Cube (ap² f (&hids p)) (&hids (ap f p)) (&hids _) (&hids _) ids ids
&aphids f idp = idc
...
ap+-reduce {i = i} {p = idp} q = &coh2 (&coh q)  where
  ...
  &coh : {X : Type i} {x y : X} {p q : x == y} {sq : Square p q idp idp} {r : p == q} (s : r == &antidegen sq) → &degen r == sq
  &coh {p = idp} {sq = ids} idp = idp

  &coh2 : {X : Type i} {x y : X} {p : x == y} {sq : Square p p idp idp} (e : &degen idp == sq) → Cube (&hids _) sq (&hids _) (&hids _) (&hids _) (&hids _)
  &coh2 {p = idp} idp = idc
...
&hinv : ∀ {i} → {X : Type i} {a b : X} {p q : a == b} (sq : Square p q idp idp) → Square q p idp idp
&hinv {p = idp} ids = ids

&aphinv : ∀ {i} → {X Y : Type i} (f : X → Y) {a b : X} {p q : a == b} (sq : Square p q idp idp) → Cube (ap² f (&hinv sq)) (&hinv (ap² f sq)) (&hids (ap f q)) (&hids (ap f p)) ids ids
...
ap+-∘1 f g idp = lemma where
  lemma : {a : _} {b : _} {p : a == b} → Cube (&hids (ap f p)) (ap² f (&hids p)) (&hids (ap f p)) (&hids (ap f p)) ids ids
  lemma {p = idp} = idc
...
&sym : ∀ {i} {A : Type i} {a b c d : A} {p : a == b} {q : c == d} {r : a == c} {s : b == d} → Square p q r s → Square r s p q
&sym ids = ids

ap-sym : ∀ {i} {A B C : Type i} (f : A → B → C) {b b' : B} (q : b == b') {a a' : A} (p : a == a') → Cube (ap+ (λ x → ap (λ y → f x y) q) p) (&sym (ap+ (λ y → ap (λ x → f x y) p) q)) (&hids (ap (f a) q)) (&hids (ap (f a') q)) (&hids (ap (λ x → f x b) p)) (&hids (ap (λ x → f x b') p))
ap-sym f idp idp = idc

-- ap₂ : ∀ {i} {A B C : Type i} (f : A → B → C) {a a' : A} (p : a == a') {b b' : B} (q : b == b') → Square (ap (f a) q) (ap (f a') q) (ap (λ x → f x b) p) (ap (λ x → f x b') p)
-- ap₂ f idp idp = ids

&apdegen : ∀ {i} → {A B : Type i} (f : A → B) {a : A} {b : A} {p q : a == b} (r : p == q) → Cube (ap² f (&degen r)) (&degen (ap (ap f) r)) (&degen idp) (&degen idp) (&degen idp) (&degen idp)
...
ap++-reduce : ∀ {i} {A B : Type i} {a b c d : A → B} {p : (x : A) → a x == b x} {q : (x : A) → c x == d x} {r : (x : A) → a x == c x} {s : (x : A) → b x == d x} {f : (x : A) → Square (p x) (q x) (r x) (s x)}
  ...
  → HyperCube (ap++ f t) cc &hidc &hidc &hidc &hidc &hidc &hidc
ap++-reduce {i = i} idp cc q = coh q  where
  ...
  coh : {X : Type i} {a b c d : X} {p : a == b} {q : c == d} {r : a == c} {s : b == d} {sq sq' : Square p q r s} {eq : sq == sq'} {cc : Cube sq sq' (&hids p) (&hids q) (&hids r) (&hids s)}
             (s : eq == &antidegen-cube cc) → HyperCube (degen-square eq) cc &hidc &hidc &hidc &hidc &hidc &hidc
  coh {sq = ids} {cc = idc} idp = idhc
...
module _ {i} {A B : Ptd i} where
  module SmashElimId {D : Type i} {g h : ∣ A ∧ B ∣ → D} (proj* : (a : ∣ A ∣) (b : ∣ B ∣) → g (proj a b) == h (proj a b))
             (basel* : g basel == h basel) (gluel* : (a : ∣ A ∣) → Square (proj* a (pt B)) basel* (ap g (gluel a)) (ap h (gluel a)))
             (baser* : g baser == h baser) (gluer* : (b : ∣ B ∣) → Square (proj* (pt A) b) baser* (ap g (gluer b)) (ap h (gluer b))) where
  ...
  module SmashElimSquare {D : Type i} {a b c d : ∣ A ∧ B ∣ → D} {p : (x : ∣ A ∧ B ∣) → a x == b x} {q : (x : ∣ A ∧ B ∣) → c x == d x} {r : (x : ∣ A ∧ B ∣) → a x == c x} {s : (x : ∣ A ∧ B ∣) → b x == d x}
             ...
             (basel* : Square (p basel) (q basel) (r basel) (s basel)) (gluel* : (a : ∣ A ∣) → Cube (proj* a (pt B)) basel* (ap+ p (gluel a)) (ap+ q (gluel a)) (ap+ r (gluel a)) (ap+ s (gluel a)))
             (baser* : Square (p baser) (q baser) (r baser) (s baser)) (gluer* : (b : ∣ B ∣) → Cube (proj* (pt A) b) baser* (ap+ p (gluer b)) (ap+ q (gluer b)) (ap+ r (gluer b)) (ap+ s (gluer b))) where
    ...
    gluel-β : (a : ∣ A ∣) → HyperCube (ap++ f (gluel a)) (gluel* a) &hidc &hidc &hidc &hidc &hidc &hidc
    gluel-β a = ap++-reduce (gluel a) (gluel* a) (M.gluel-β a)

    gluer-β : (b : ∣ B ∣) → HyperCube (ap++ f (gluer b)) (gluer* b) &hidc &hidc &hidc &hidc &hidc &hidc
